            ...
            let 단일태그 = `<a href="./detial.html?number=${index}">
                            <div id="카드${index}" class="카드">
                                <div id="카드이미지${index}" class="카드이미지" style="background-image: url(${image_url_})">
                                    <button id="${index}번째" class="삭제버튼" onclick='카드삭제기능(event)'>X</button>
                                </div>                            
                                    <div id="" class="카드내용요약">
                                        <div class="첫줄">
                                            <div class="기분노출">
            ...
            let 단일태그 = `<a href="./detial.html?number=${index[i]}">
                                <div id="카드${index[i]}" class="카드">
                                        <div id="카드이미지${index[i]}" class="카드이미지" style="background-image: url(${image_url_})">
                                            <button id="${index[i]}번째" class="삭제버튼" onclick='카드삭제기능(event)'>X</button>
                                        </div>                            
                                        <div id="" class="카드내용요약">
                                            <div class="첫줄">
                                                <div class="기분노출">
...
const 카드삭제기능 = (event) =>{
    ...
    let 삭제할인덱스배열 = []
    삭제할인덱스배열.push(event.taget.id)
    let 인덱스배열 = () =>{
        const 전체인덱스 = []
        for(let i=0; i < 인덱스배열.length; i++){
            전체인덱스.push(i)
        }
        return 전체인덱스
    }
    const 남은인덱스 = 전체인덱스.filter((ele)=> !(삭제할인덱스배열.includes(ele)) )
    console.log("남은인덱스", 남은인덱스)
    카드태그생성(남은인덱스)
    for (let i =0; i< 삭제할인덱스배열; i++){
        JSON.parse(localStorage.getItem("일기카드들").removeItem(삭제할인덱스배열[i]))
    }
}
